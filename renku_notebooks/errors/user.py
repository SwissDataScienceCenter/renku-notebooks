...
class MissingResourceError(UserInputError):
    """Raised when any type of resource (either in k8s or outside the cluster) does not exist.

    Only raised if it is expected that it does exist. This is also raised when the user
...
class AuthenticationError(UserInputError):
    """Raised when the user needs to be authenticated to access a specific resource.
...
class ImageParseError(UserInputError):
    """Raised when an invalid docker image name is provided.
...
class InvalidComputeResourceError(UserInputError):
    """Raised when invalid server options are requested.

    Also raised when the user has no access to a resource class.
    """
