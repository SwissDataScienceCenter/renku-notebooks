...
def get_single_server_response_schema(config: NotebooksConfig):
        ...
        def format_user_pod_data(self, server, *args, **kwargs):
            ...
            def get_failed_message(failed_containers):
                ...
                for container in failed_containers:
                    ...
                    if (
                        ...
                        user_correctable_message = get_user_correctable_message(
                            exit_code
                        )
            ...
            def get_resource_requests(server):
                ...
                if "cpu_request" in server_options_keys:
                    resources["cpu"] = CpuField().deserialize(
                        server_options["cpu_request"]
                    )
            ...
            output = {
                ...
                "state": {
                    "pod_name": server.js["status"].get("mainPod", {}).get("name")
                },
    ...
    if config.s3_mounts_enabled:
        return LaunchNotebookResponseWithS3
    return LaunchNotebookResponseWithoutS3


def get_servers_response_schema(config: NotebooksConfig):
    response_schema = get_single_server_response_schema(config=config)
    ...
    class ServersGetResponse(Schema):
        ...
        servers = fields.Dict(
            keys=fields.Str(),
            values=fields.Nested(response_schema),
        )
