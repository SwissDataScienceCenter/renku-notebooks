...
bp = Blueprint(
    'notebooks_blueprint', __name__, url_prefix=config.SERVICE_PREFIX
)
...
@bp.route(
    '<namespace>/<project>/<commit_sha>/<path:notebook>', methods=['GET']
)
@authenticated
...
@bp.route(
    '<namespace>/<project>/<commit_sha>/<path:notebook>', methods=['POST']
)
@authenticated
def launch_notebook(user, namespace, project, commit_sha, notebook=None):
    ...
    if get_project_permissions(user, gl_project) < DEVELOPER_ACCESS:
        return current_app.response_class(
            status=401,
            response=
            'Not authorized to use interactive environments for this project.'
        )
