...
@bp.route("renku-2-servers", methods=["POST"])
...
def launch_notebook_helper(
    ...
    cloudstorage,
    server_options,
    renku_2: bool,  # Renku 2
    project_id: Optional[str],  # Renku 2
    environment_id: Optional[str],  # Renku 2
    ...
    if renku_2:
        ...
        repository = Repository.from_schema(repositories[0]) if repositories else None
        gl_project = user.get_renku_project(f"{repository.namespace}/{repository.project}") if repository else None
    else:
