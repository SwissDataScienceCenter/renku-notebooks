...
func getTestConfig(renkuUrl string, gitAccessToken string, gitAccessTokenExpiresAt int64, renkuAccessToken string) GitProxyConfig {
	...
	os.Setenv("RENKU_REFRESH_TOKEN", "SomeRefreshToken")
...
func TestSuccessfulRefresh(t *testing.T) {
	...
	newRenkuTokenExpiresAt := time.Now().Unix() + 3600
	newRenkuToken, err := getDummyAccessToken(newRenkuTokenExpiresAt)
	assert.Nil(t, err)
	oldRenkuToken, err := getDummyAccessToken(time.Now().Unix() - 3600)
	assert.Nil(t, err)
	...
	evenNewerRenkuToken, err := getDummyAccessToken(time.Now().Unix() + 3600)
	assert.Nil(t, err)
...
func TestNoRefreshNeeded(t *testing.T) {
	...
	renkuAccessToken, err := config.getAndRefreshRenkuAccessToken()
	assert.Nil(t, err)
	assert.Equal(t, renkuAccessToken, oldRenkuToken)
}
